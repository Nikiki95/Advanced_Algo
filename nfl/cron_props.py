#!/usr/bin/env python3
"""NFL player props runner with tracking, risk controls and optional player priors."""
...
from player_props_scraper import NFLPlayerPropsScraper
...
class NFLPropsCronRunner:
    def __init__(self, execution_mode: str = 'live'):
        self.execution_mode = execution_mode
        self.scraper = NFLPlayerPropsScraper()
    ...
    def _record_snapshots(self, event):
        ...
        self.closing.record_candidates('nfl', event['home_team'], event['away_team'], event.get('match_date'), event['event_id'], rows)
        ...
        for event in events:
            ...
            for (market, player_name, line), options in self._group_rows(event.get('rows', [])).items():
                candidate = analyze_over_under_group(
                    ...
                    event_id=canonical_event_key('nfl', event['home_team'], event['away_team'], event.get('match_date')),
                ...
                if not candidate:
                    continue
                calibration = self.calibration.adjust_bet('nfl', candidate.market, candidate.raw_kelly_stake, candidate.value_percentage, candidate.confidence)
                payload = candidate.to_tracking_payload(self.execution_mode, 'nfl-props-consensus-v1', self.calibration.profile.get('version', 'v4-props'))
                ...
                payload['event_id'] = canonical_event_key('nfl', event['home_team'], event['away_team'], event.get('match_date'))
...
def main():
    parser = argparse.ArgumentParser(description='NFL player props runner with tracking, risk controls and optional player priors.')
    parser.add_argument('--markets', nargs='*', default=None)
    parser.add_argument('--max-games', type=int, default=None)
    parser.add_argument('--shadow', action='store_true')
    args = parser.parse_args()
    runner = NFLPropsCronRunner(execution_mode='shadow' if args.shadow else os.getenv('EXECUTION_MODE', 'live'))
